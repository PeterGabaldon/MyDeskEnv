# Generated by Powerlevel10k configuration wizard on 2020-09-16 at 11:49 CEST.
...
function prompt_my_ram_used_percentage() {
	local integer memavailable=$(cat /proc/meminfo | grep -i memavailable | awk '{print $2}')
	local integer memtotal=$(cat /proc/meminfo | grep -i memtotal | awk '{print $2}')

	local integer percentage=$(echo "(1-($memavailable / $memtotal))*100" | bc -l | cut -c 1-5)

	if (( percentage <= 25 )); then
		local integer state=1
	elif (( percentage <= 50 )); then
		local integer state=2
	elif (( percentage <= 75 )); then
		local integer state=3
	else
		local integer state=4
	fi
	
	p10k segment -s $state -f '#aac4a0' -i '' -t "${percentage}%%"

}
POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS+=my_ram_used_percentage
POWERLEVEL9K_MY_RAM_USED_PERCENTAGE_1_FOREGROUND='#bad6b1'
POWERLEVEL9K_MY_RAM_USED_PERCENTAGE_2_FOREGROUND='#da7575'
POWERLEVEL9K_MY_RAM_USED_PERCENTAGE_3_FOREGROUND='#c66a3f'
POWERLEVEL9K_MY_RAM_USED_PERCENTAGE_4_FOREGROUND='#df1a1a'

function prompt_my_ip_tun0() {
	local string ip=$(/usr/sbin/ifconfig tun0 2> /dev/null | grep -w inet | awk '{print $2}')

	p10k segment -c "${ip}" -f '#f0f0f0' -i '' -t "${ip}"
}
POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS+=my_ip_tun0
